...
| Latest local candidate              | Current branch adds the disabled reporting pipeline through controlled private deletion; it is not public |
...
| Automated release gate              | Node.js 24; 415/415 tests across 49 files; typecheck, lint, and production build pass                     |
...
| Automated verification              | PASS                  | Node.js 24 ran 415 tests across 49 files plus typecheck, lint, build, deterministic Local Guard packaging, and real D1 migration/intake/review/publication/signed-feed/retention/legal-hold/deletion gates. A fresh `npm ci` remains part of final freeze.                                                                                                                                                                                            |
...
| Reporting                           | PRIVATE PIPELINE OFF  | Strict intake/reviewer/publisher handlers, immutable minimized publication, signed JSON/NDJSON feed, atomic retention assignment, custodian-only legal hold, and controlled private deletion with non-identifying tombstones exist locally; all remain disabled/unconfigured publicly. Public corrections, production identity/key custody, separately published trust fingerprint, provider-backup lifecycle, and operations approval remain absent. |
