...
> records the explicitly deployed novice release and the version 12
> reconciliation. A September 3 cleanup candidate retires anonymous receipt
> upload, keeps lesson receipts private to the page session unless exported,
> and bounds invocation data. Local Guard disclosure pages are public; the
> extension, connector, private reporting operations, moderation, and Android
> remain outside the public deployment.

**Report date:** 2026-09-03
**Scope:** current local cleanup candidate, historical public/Sites-source
commit `d0c1676`, version 10/11 browser evidence, and the undeployed Local
Guard extension, connector, reporting operations, moderation, and Android
candidate
...
cleanup candidate later passed a fresh `npm ci` followed by the complete local
productization gate:
typecheck, Local Guard and reporting disclosure/readiness assessments, an inert
standalone reporting-Worker dry run, 551 passing tests across 76 files, lint, the
production build, and a fail-closed inspection of the generated deployment
bundle. That post-build check allows only generated client/server artifacts and
reviewed hosting/migration inputs; requires internal `.vite` metadata to remain
excluded from public assets; rejects raw source, source maps, repository-only
paths, unexpected artifact types, known credential forms, and added Worker
authority; and enforces bounded file counts and sizes. The
...
`SKIP_UNSUPPORTED_WORKSPACE`; external browser and Left Out Membrane runs are
...
| Current productization candidate  | Pass with explicit asset blocker | The cleanup candidate passes 551 tests across 76 Vitest files; typecheck, Local Guard and reporting release-readiness assessments, inert standalone reporting-Worker dry run, lint, production build, post-build public-`dist/` allowlist verification, deterministic package generation, stale Store-asset detection, trusted-key attestation verification, source-ready identity-bound native transport, a separately packaged no-host-permission native extension candidate, authenticated replay-resistant named-pipe IPC/native-only connector mode, lifecycle, platform-matrix, incident-response, default-off four-field report-relay checks, loopback reviewer authority/accessibility-contract checks, reporting-role separation, bounded public-page invocation data, private session-only lesson receipts, retirement of the anonymous evidence endpoint, real D1 migration enforcement for the disabled reporting service, isolated Worker routing, routable-but-disabled infrastructure compilation, retention-preserving idempotency replay, legacy schema-v1 hash-chain compatibility, stale-snapshot deletion-race rejection, and disabled-first intake/review/publication/correction/signed-feed/retention/legal-hold/deletion checks passed. The Local Guard Store screenshot must be recaptured after the public brand correction and is not release-ready. |
...
| --------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Public novice URL                                   | Published candidate         | Live content strongly matches commit `d0c1676`, Sites version 12. It contains the setup gate, walkthrough, lessons, and Local Guard disclosure pages; reporting APIs are fail-closed 404 and no Local Guard extension, connector, reporting operations, or Android product is hosted.                                                                                                                                                                        |
...
| Extension                                           | Limited                     | Manifest V3 `0.3.0` completed one five-lesson unpacked run. A deterministic preview package, release-attestation gate, and separate unsigned `0.4.0` native-only extension source candidate now exist locally; no native-host executable, installed native manifest, production key, Chrome-signed, or store-distributed package was tested.                                                                                                                 |
| Android                                             | Conformance only            | A September 3 checkpoint passed all eight Kotlin/JVM groups and the API-36 adapter compile/manifest check; `adb devices -l` returned no connected device. The later disclaimer compatibility patch was not rerun through that separate Windows/Android gate in the final Linux executor. This does not establish exact-final-candidate Android verification, generated AppFunction metadata, or on-device registration, discovery, invocation, and receipts. |
...
- Public page receipts are session-local and export-only. Historical D1 rows,
  if any, require a separate operator-approved retention or purge decision.
...
- The public application exposes no receipt-upload route. Exported client JSON
  remains self-reported evidence, not trusted provenance.
