...
standalone reporting-Worker dry run, 533 passing tests plus two Windows-only skips across 75 files, lint, the
...
| Android                                             | Conformance only            | On September 3, all eight Kotlin/JVM groups and the API-36 adapter compile/manifest check passed; `adb devices -l` returned no connected device. This does not establish generated AppFunction metadata or on-device registration, discovery, invocation, and receipts.                                                                      |
