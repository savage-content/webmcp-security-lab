...
- A disabled-by-default, privacy-minimized reporting core with a strict invited
  ...
  bounded publication-and-correction timeline pages with an externally supplied
...
After a clean `npm ci` on September 3, the current local candidate passed 525
automated tests across 73 files on Node.js 24. The complete gate also passes
...
- Reporting remains locally implemented and publicly disabled. Invited HTTP
  ...
  provider-backup purge, operator recovery, correction rehearsal, and privacy
  approval do not. Immutable public withdrawal exists only as disabled,
  source-tested code.
