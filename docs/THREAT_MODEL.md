...
| Native IPC message alteration, replay, or credential exposure | A local process impersonates the native host or replays a prior command                     | Strict HMAC-SHA-256 envelope, install-scoped secret, exact request/response binding, nonce and timestamp window, bounded replay cache and frames, one request per pipe connection, five-action adapter, and no retry; signed installer, protected secret storage, verified pipe ACL, and hostile-local-process acceptance remain external gates |
...
- The source-ready native-messaging checkpoint binds one exact extension
  origin, closes and bounds its message schemas, connects a native-only
  connector mode through authenticated replay-resistant named-pipe IPC, keeps
  coordinator bridge tokens out of browser state, and models an HKCU install
