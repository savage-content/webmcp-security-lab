...
| Working live project           | Judges must be able to use the intended platform through ChatGPT's in-app browser or Chrome 149+ with WebMCP enabled.                                             | A public Sites URL exists. Its exact deployed commit and a fresh unauthenticated Site Tools run must be frozen and recorded.                              | **OPEN**                                    |
...
| Public source repository       | GitHub, GitLab, or Bitbucket; all required code, assets, and instructions; visible open-source license; actual `document.modelContext.registerTool()` use.        | The GitHub repository is public and recognizes the MIT license, but its public branch does not yet contain the current candidate or submission materials. | **BLOCKED**                                 |
