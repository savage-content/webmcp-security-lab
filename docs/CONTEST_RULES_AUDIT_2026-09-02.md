...
| Public source repository       | GitHub, GitLab, or Bitbucket; all required code, assets, and instructions; visible open-source license; actual `document.modelContext.registerTool()` use.        | Public GitHub contains the version 12 source and submission materials at `d0c1676`; GitHub detects the MIT license. Record the final reviewed patch SHA.    | **PASS**                                    |
